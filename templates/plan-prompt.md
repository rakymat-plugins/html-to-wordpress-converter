...
If the source has blog/news/articles/press content, or the WordPress implementation uses standard posts, the plan must include branded post templates: `home.php` for the Posts page, `archive.php` for post archives, `single.php` for individual posts, shared post card/pagination partials, and matching Sage Blade views when `resources/views` exists. These templates must reuse the converted site's branding colors, fonts, cards, buttons, global header/footer, and responsive spacing.
